...
export default function BidList({ user, auction }: Props) {
    ...
    // state variable to check if the auction is open for bids
    const open = useBidStore(state => state.open);
    const setOpen = useBidStore(state => state.setOpen);
    const openForBids = new Date(auction.auctionEnd) > new Date(); // compared from aution timer with current date
    // to check if auction can be made
    ...
    // set the open state that determines if bids can be made
    useEffect(() => {
        setOpen(openForBids);
    }, [openForBids, setOpen])
